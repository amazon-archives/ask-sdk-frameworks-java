...
import static com.amazon.ask.models.codegen.Utils.validateNamespace;
        ...
        for (IntentDefinition intentDefinition : intents.keySet()) {
            ...
            if (data.values().stream().anyMatch(d -> !d.isEmpty())) {
                for (Map.Entry<Locale, IntentData> entry : data.entrySet()) {
                    ...
                    MAPPER.writer(com.amazon.ask.models.Utils.PRETTY_PRINTER).writeValue(dataFile, entry.getValue());
                }
            }
        }
